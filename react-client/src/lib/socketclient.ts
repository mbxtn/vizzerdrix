...
export function createVdClient(serverUrl?: string): VdClient {
    ...
    if (serverUrl) {
        ...
        // Development mode: React dev server is on 4200, so connect to server on 3000
...
function restorePrototypes(obj: any): any {
    ...
    if (obj.players && obj.turnOrder && obj.currentTurn !== undefined) {
        ...
        if (obj.players) {
            Object.keys(obj.players).forEach(playerId => {
                ...
                // Restore prototypes for cards in each player's cards dictionary
                if (player.cards) {
    ...
    } else if (obj.name && obj.cards) {
        // This looks like a Player object
        Object.setPrototypeOf(obj, Player.prototype);
        
        // Restore prototypes for cards in the cards dictionary
        if (obj.cards) {
            Object.keys(obj.cards).forEach(cardId => {
                const card = obj.cards[cardId];
                Object.setPrototypeOf(card, Card.prototype);
            });
        }
